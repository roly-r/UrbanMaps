...
guias_bp = Blueprint('guias', __name__, template_folder='templates')
...
def index():
    ...
    cur=conn.cursor()
    cur.execute("SELECT * FROM guia ORDER BY fecha_incorporacion DESC")
    afiliados=cur.fetchall()
    return render_template("index_afiliado.html",afiliados=afiliados)

@guias_bp.route("/detalle/<int:ci>")
@login_required
def detalle_afiliado(ci):
    ...
    # Datos del afiliado
    cur.execute("SELECT * FROM guia WHERE ci = ?", (ci,))
...
@guias_bp.route("/crear/guarda",methods=['POST'])
@login_required
def afiliado_sv():
    if not verifica():
    ...
    cur.execute("INSERT INTO guia (ci,nombres,apellidos,direccion,fecha_nacimiento,telefono,fecha_incorporacion,nro_licencia) VALUES (?,?,?,?,?,?,?,?)",
...
@guias_bp.route("/update",methods=['POST'])
@login_required
def afiliados_update():
    ...
    cu.execute("UPDATE guia SET nombres=?,apellidos=?,direccion=?,fecha_nacimiento=?,telefono=?,fecha_incorporacion=?,nro_licencia=? WHERE ci=?",
...
@guias_bp.route('/buscar_afi', methods=['GET', 'POST'])
@login_required
def buscar_afi():
    if request.method == 'POST':
        ...
        if not termino:
            ...
            return redirect(url_for('guias.buscar_afi'))

        conn = sqlite3.connect("urban_maps.db")
        conn.row_factory = sqlite3.Row  # Importante para acceder por nombre
        cursor = conn.cursor()
        cursor.execute("""SELECT * FROM guia WHERE ci = ? OR nombres = ?""", (termino, termino))
...
def afiliados_del(ci):
    ...
    cur.execute("DELETE FROM guia WHERE ci=?",(ci,))
    conn.commit()
    conn.close()
    return redirect(url_for('guias.index'))
...
@guias_bp.route('/reporte_detalle/<int:ci>', methods=['GET'])
@login_required
def generar_reporte_detalle(ci):
    try:
        ...
        if not afiliado:
            flash("No se encontró al guias con el CI proporcionado.", "error")
            return redirect(url_for('guias.index'))
